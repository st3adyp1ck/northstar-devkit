@{
    ...
    Items = @(
        ...
        @{
            ...
            Help       = "View, add, remove, and comment-toggle entries in the Windows hosts file (%windir%\System32\drivers\etc\hosts). Use -Show (or the menu's default listing) for a read-only numbered report showing each entry's IP, hostnames, and commented state; run with no flags for an interactive [A]dd/[R]emove/[T]oggle/[Q]uit menu, or -Add ""ip hostname..."" / -Remove / -Toggle (by listing index or hostname pattern) for scripted changes. Use this to map local dev domains (e.g. 127.0.0.1 mysite.local) without opening Notepad as admin. Safety note: every mutation requires Administrator privileges (already satisfied inside always-elevated DevKit; the script still offers to relaunch itself elevated if invoked outside DevKit), is gated by the shared confirmation prompt unless -Force is passed, creates a timestamped backup under %LOCALAPPDATA%\NorthstarDevKit\hosts-backups\ before the first write of a session, writes the file back as plain ASCII preserving all comments and ordering, and clears the DNS client cache afterward. -Show never mutates anything. This menu item runs with -Show, so opening it from the DevKit menu gives the read-only listing without requiring admin elevation."
