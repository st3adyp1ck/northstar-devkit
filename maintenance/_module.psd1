@{
    Name        = "Maintenance"
    Description = "Real Windows maintenance and tuning: disk/storage cleanup, startup and service management, system file repair (SFC/DISM), Windows Update cache reset, scheduled tasks and event log review, power and visual-effects tuning, and hardware health reporting - distinct from diagnostics/, which checks dev-tool health rather than the OS itself. Most mutating tools default to a safe, non-mutating report and only change anything real once you pass an explicit flag and confirm; a few (system file repair, Windows Update reset, memory diagnostic) attempt their real action by default behind just a confirmation prompt, so check an item's Help below before running it unattended."
    Items       = @(
        @{
            Key    = '1'
            Label  = 'Clear Disk Junk (temp/WU cache/Recycle Bin)'
            Script = 'Clear-DiskJunk.ps1'
            Help   = "Scans user/system temp folders, the Windows Update download cache, and the Recycle Bin, and reports how much space each is holding. Use this to see how much junk is reclaimable before deciding whether to clean it. Safety note: bare invocation only reports - nothing is deleted unless you pass -Apply, which then requires typing 'CLEAN' to confirm (or -Force to skip that prompt). -Apply also stops/restarts the Windows Update services around clearing its cache. -IncludeWinSxS additionally runs 'Dism.exe /Online /Cleanup-Image /StartComponentCleanup' and requires an elevated session."
        }
        @{
            Key    = '2'
        ...
        @{
            ...
            Help   = "Lists Windows scheduled tasks with their state, last run time, and last result, flagging any nonzero result code, and can disable a single named task. Use -NonMicrosoftOnly to filter out the \Microsoft\ task-folder tree and focus on third-party tasks. Safety note: bare invocation and -NonMicrosoftOnly are read-only (no admin rights needed to list). -Disable mutates - it requires confirmation unless -Force is passed, and may itself require an elevated session depending on the task's owning scope."
        }
        @{
            Key    = '8'
        ...
        @{
            ...
            Help   = "Lists Windows power plans (via powercfg /list) with the active one marked - this is also the bare/default behavior - switches the active plan by name via -SetPlan, and can unlock the hidden 'Ultimate Performance' plan via -UnlockUltimate. Use this to see or change which power plan is active, or to make Ultimate Performance available to switch to. Safety note: bare invocation and -List are read-only. -SetPlan requires confirmation unless -Force is passed. -UnlockUltimate requires an elevated (Administrator) session plus confirmation unless -Force is passed."
        }
        @{
            Key    = '10'
            Label  = 'Visual Effects Manager'
            Script = 'Set-VisualEffects.ps1'
            Help   = "Reports or sets the Windows 'adjust for best performance/appearance' visual effects mode (Performance, Appearance, Auto, or Custom) - the setting behind Advanced System Settings -> Performance. Use this to quickly trade visual polish for responsiveness or vice versa. Safety note: bare invocation and -Show are read-only. -SetMode mutates the current user's own HKCU registry hive (no admin rights needed) and requires confirmation unless -Force is passed; a sign-out or Explorer restart may be needed for all effects to visibly update."
        }
        @{
            Key    = '11'
