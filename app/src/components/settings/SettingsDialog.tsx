...
import { clampUiScale } from "../../hooks/useApplyAppearance";
...
function AppearanceSection({ prefs, update, queue }: SectionProps & { queue: QueueFn }) {
  ...
  function onScaleChange(percent: number) {
    ...
    setScaleDraft(scale);
    // Instant preview - the same write useApplyAppearance makes once the
    // debounced save lands, so the two can never disagree for long.
