...
.DESCRIPTION
    ...
    no tests/ (Pester isn't needed at runtime), no dev/ (maintainer-only
    tooling), no editor folders.
...
.PARAMETER OutputDir
    Where to build the portable folder. Defaults to USB/ at the repo root.
.EXAMPLE
...
# This script lives in dev\ (maintainer tooling) - the repo root it mirrors
# is its parent folder.
$RepoRoot = Split-Path -Parent $PSScriptRoot
. (Join-Path $RepoRoot "tools\lib\DevKit-Common.ps1")
